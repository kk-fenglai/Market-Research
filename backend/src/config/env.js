...
// 市场调研 AI 提供方(经济版仅需 DeepSeek;均衡/高级版另需 Perplexity / Anthropic)。
// 调用时若缺 key 各 provider 自行报错,不在 boot 阻塞 —— dev 仅告警。
requireEnv('DEEPSEEK_API_KEY', { minLength: 30, prodOnly: true });
if (!IS_PROD && !process.env.DEEPSEEK_API_KEY) {
  warnings.push('DEEPSEEK_API_KEY 未设置 — 经济版调研将无法运行(均衡/高级版另需 PERPLEXITY/ANTHROPIC key)');
}
